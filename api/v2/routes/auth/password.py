...
async def login_by_code(
    ...
    # Optional Mini App ↔ Telegram link: caller opened a `?link_token=` URL
    # generated by the tgapp user. Attach this email to *their* identity (the
    # Telegram-bound one) and continue the session as the merged identity.
    if body.link_token:
        from utils.identity_link import LINK_KIND_WEB, consume_link_token
        ...
        if remote_id and remote_id != str(identity.id):
            ...
            if merged is None:
                raise HTTPException(
                    status_code=409,
                    detail="На обоих аккаунтах оформлены подписки — связать нельзя",
                )
            identity = merged
    await bind_identity_actor(request, session, identity)
